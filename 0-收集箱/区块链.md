...
Mermaid 渲染器缺少 `mmdc`，第一次制图无法完成验证。我已改用 SVG 方案重试，仍会经过渲染检查后再嵌入，避免放入未经验证的图。
...
![[viz-blockchain-transaction-state-flow-1790165002124.png|700]]
...
`/Users/hongling/learn/viz/viz-blockchain-transaction-state-flow-1790165002124.png`

但 Obsidian 当前打开的 vault 很可能不是 `/Users/hongling/learn`，所以无法通过文件名找到它。

你的 Obsidian 笔记文件实际位于哪个路径（或 vault 根目录是什么）？
...
🥷 已修复：图片已复制到你的 Obsidian vault：

`~/obsidian-second-brain/_附件/区块链/viz-blockchain-transaction-state-flow-1790165002124.png`

请将原来的引用替换为：

```markdown
![区块链交易到状态流程](_附件/区块链/viz-blockchain-transaction-state-flow-1790165002124.png)
```

或使用 Obsidian wikilink：

```markdown
![[区块链/viz-blockchain-transaction-state-flow-1790165002124.png|700]]
```

根因是图片原先位于 `/Users/hongling/learn/viz/`，不在你的 Obsidian vault 内。
