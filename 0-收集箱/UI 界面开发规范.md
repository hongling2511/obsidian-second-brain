...
设计系统最终服务的是**任务成功率和体验质量**，不是截图相似度或组件数量。

至此，“创建记录”已经验证了跨平台规范的核心结构：
...
下一步可以进入基础层：为这套系统定义**颜色、字体、间距、圆角、层级和动效等设计令牌**，并继续区分哪些令牌跨平台共享、哪些需要平台化。
...
```text
按钮背景 → color.action.primary
页面背景 → color.surface.default
错误提示 → color.feedback.error
```
...
它们可能颜色相近，但语义不同，未来也可能被分别调整。

### 间距也应该表达关系

不要只在每个地方随意写 `16px`。可以定义：

```text
space.label-to-input
space.field-to-field
...
```text
space.touch-target
space.desktop-density
space.mobile-page-padding
```

移动端可能需要更宽松的触控空间，桌面端可能允许更高的信息密度，但它们仍然属于同一套语义体系。

小测验：为什么组件应该优先使用 `color.action.primary`，而不是直接使用 `blue-500`？

- A. 语义令牌名称更短
- B. 语义令牌能表达用途，并支持主题和平台调整
- C. 原始颜色值无法被设计工具读取
- D. 语义令牌会自动生成所有组件<style>
...
语义令牌把组件和具体颜色解耦，方便主题、平台和无障碍模式调整。
...
占位提示不能代替字段标签，因为用户输入后，占位提示会消失。

小测验：为什么不能只用字号来定义字体系统？

- A. 字号在不同设备上无法显示
- B. 同一字号承担的语义和阅读任务可能不同
- C. 字号会影响颜色
- D. 字号只能用于标题，不能用于正文
...
小测验：如果页面标题与字段之间使用的间距，和标签与输入框之间完全相同，最可能出现什么问题？
...
下一步可以讨论设计系统最容易被忽略的部分：**如何把 Figma 规范、代码组件和平台实现保持一致**。
...
1. **视觉测试**：布局、颜色、状态是否正确
2. **交互测试**：键盘、触控、加载、错误、重试
3. **无障碍测试**：焦点、读屏、对比度、语义结构
...
> **设计系统的每条规范，都应该能被实现、被测试、被版本化。**

小测验：如果设计稿里的按钮已经支持“加载中”，代码组件最合理的做法是什么？

- A. 只在设计稿中记录，不要求代码实现
- B. 由每个业务页面自行决定加载表现
- C. 在组件 API 和测试中明确支持加载状态
- D. 删除加载状态，避免组件变复杂
...
加载状态必须进入组件契约，否则每个业务都会实现出不同版本，最终导致：
...
这套 UI 设计规范的核心不是“三端长得一样”，而是：

> **用户在任何平台上，都能用符合该平台习惯的方式，可靠完成同一个任务。**

接下来可以进入更具体的实践：为“创建记录”定义一份完整的按钮、输入框、错误提示和保存状态规范。


> [!quote] YOU

好的 我已经有了基本的了解了，现在我们可以选择哪些开源的设计系统二次开发
...
- **完整组件库**：拿来即用，适合快速开发
- **无头组件 / 基础原语**：提供可访问性和交互逻辑，适合深度定制

## 推荐候选

| 方案 | 适合场景 | 定制能力 |
|---|---|---|
| **shadcn/ui + Radix UI** | 高度定制的 SaaS、后台、现代 Web | 很高 |
| **MUI** | React 企业应用、完整后台 | 中高 |
| **Ant Design** | 数据密集型后台、管理系统 | 中等 |
| **Mantine** | React 项目、快速开发、体验平衡 | 高 |
| **Chakra UI** | 快速搭建、主题化、可访问性 | 高 |
| **Fluent UI** | 桌面感、企业办公、Microsoft 风格 | 中高 |
| **Carbon Design System** | 企业软件、数据和复杂工作流 | 中高 |
| **React Aria** | 高可访问性、自己掌控视觉系统 | 很高 |
| **Headless UI** | Tailwind 项目、交互原语 | 很高 |
| **PatternFly** | 企业后台、运维、云平台 | 中等 |
...
优点是组件源码直接进入项目，可以自由改造，不容易被组件库默认风格限制。

代价是：团队要自己维护组件、升级和一致性。
...
表格、表单、筛选、分页、弹窗、国际化等都比较完整。

代价是：容易产生明显的 Ant Design 风格，深度定制需要熟悉它的主题机制。

### 3. 想要成熟、全面、稳妥的 React 方案

```text
MUI
```

生态成熟、组件丰富、文档和社区都强，适合企业级 React 应用。

代价是：如果品牌视觉和 Material 差异很大，定制成本会增加。
...
API 现代，组件和 Hooks 都比较丰富，适合中小团队快速构建产品。
...
更适合侧栏、多窗口、工具栏、命令、表格等办公软件语境。

### 6. 想把视觉完全掌握在自己手里

```text
React Aria / Radix UI + 自定义设计系统
```
...
不要期待一个 Web 组件库直接覆盖全部平台。更合理的架构是：
...
+ 共享状态模型
        ↓
Web：shadcn / MUI / Ant Design
移动端：Material 3 / SwiftUI 规范
桌面端：Fluent UI / 原生平台控件
```
...
1. **定制型产品**：`shadcn/ui + Radix UI`
2. **企业后台**：`Ant Design`
3. **跨平台办公产品**：`Fluent UI`

不建议一开始混用 MUI、Ant、Chakra 等多个完整组件库，容易出现视觉、主题、交互和无障碍行为不一致。
